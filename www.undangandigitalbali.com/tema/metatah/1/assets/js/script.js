(function ($) {
  ...
  $countdown.countdown(
    $date,
    function (event) {
      $(".js-countdown-days").html(
        event.strftime("%D")
      );
      $(".js-countdown-hours").html(
        event.strftime("%H")
      );
      $(".js-countdown-minutes").html(
        event.strftime("%M")
      );
      $(".js-countdown-seconds").html(
        event.strftime("%S")
      );
    }
  );
  ...
  function setActiveNavLink() {
    ...
    for (
      var i = 0;
      i < sectionsInfo.length;
      i++
    ) {
      if (
        scrollPosition >=
        sectionsInfo[i].offset
      ) {
        $(".js-nav-link").removeClass(
          "active"
        );
        $(
          '.js-nav-link[href="#' +
  ...
  var videonya =
    document.getElementById("videonya");
